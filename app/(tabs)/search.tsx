...
export default function SearchScreen() {
  const { city } = useLocalSearchParams(); // pre-filled city from Dashboard
  ...
  useEffect(() => {
    setProperties([
      { id: '1', title: '2BHK in ' + (searchValue || 'Delhi'), price: '₹50,000' },
      { id: '2', title: '3BHK Luxury in ' + (searchValue || 'Delhi'), price: '₹75,000' }
    ]);
  }, [searchValue]);

  const handleApplyFilters = (newFilters) => {
    setShowFilters(false);
    // You can filter properties based on filters here
  };
...
const styles = StyleSheet.create({
  ...
  filterButton: { justifyContent: 'center' },
